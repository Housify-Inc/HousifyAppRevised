...
class House:
    def __init__(
        self, property_address=None, property_owner=None, group=None, real_estate=None
    ):
        self.property_address = property_address
        self.property_owner = property_owner
        self.group = group
    ...
    def from_dict(cls, house_dict):
        ...
        real_estate = RealEstate(
            ...
            image=real_estate_dict.get("image"),
    ...
    def to_dict(self):
        return {
            ...
            "real_estate": {
                ...
                "image": str(self.real_estate.image),
    ...
    def retrieve_housing_info(
        self, property_address
    ):  # access database for complete user info
        ...
        if not house:
            raise HouseNotFoundException(
                f"No house found with the address: {property_address}"
            )
    ...
    def add_house_info(self):  # inserts user info into database
        ...
        file_id = fs_bucket.upload_from_stream(
            self.real_estate.image.filename,
            self.real_estate.image,
            metadata={
                "contentType": self.real_estate.image.content_type,
            },
        )

        self.real_estate.image = file_id
    ...
    def update_housing_info(self):
        ...
        existing_house = collection.find_one(
            {"property_address": self.property_address}
        )

        if not existing_house:
            raise HouseNotFoundException(
                f"No house found with the address: {self.property_address}"
            )

        # Update the house information in the database
        collection.update_one(
            {"property_address": self.property_address}, {"$set": self.to_dict()}
        )
